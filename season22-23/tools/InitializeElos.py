...
def main(pgc, eloParams):
    ...
    for team in teamResults:
        updateStatement = f"UPDATE {schemaName}.teams SET current_elo=%s WHERE team_id=%s;"
        cur.execute(updateStatement, (default_elo, team[1]))
        conn.commit()
    ...
    for conf in confResults:
        updateStatement = f"UPDATE {schemaName}.conferences SET conference_elo=%s WHERE conference_id=%s;"
        cur.execute(updateStatement, (default_elo, conf[0]))
        conn.commit()
